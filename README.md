...
> • **17대 전수 점검 룰셋**: 프롬프트 주입, DB 직접 쿼리, 고객 PII 유출, AWS IMDS SSRF 정밀 진단  
> • **비침습적 런타임 감사**: Spring AOP 기반 도구 호출 파라미터 및 DB 쿼리 무누수 추적  
> • **원클릭 조치·재검증**: 취약 상태(FAIL 90점) -> 가드레일 적용 -> 1초 재점검(PASS 0점) 자동 종결  
> 

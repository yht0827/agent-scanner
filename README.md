...
> **AgentScanner는 AI 에이전트가 도구(Tool Calling)를 통해 데이터베이스나 서버 시스템을 침해하는 연쇄 공격을 탐지하고, 가드레일 조치와 재검증(Re-Test)까지 자동화하는 보안 플랫폼입니다.**
> 
> • **17대 보안 룰셋**: OWASP LLM Top 10 기반 프롬프트 주입, 도구 오용, 민감 데이터 유출 정밀 진단  
> • **런타임 실행 추적**: Spring AOP 기반 도구 호출 파라미터 및 DB 쿼리 무누수 감사  
> • **원클릭 조치·재검증**: 취약 상태(FAIL) -> 가드레일 적용 -> 재점검(PASS) 자동 종결  
> 
> *모든 진단은 호스트 시스템 보호를 위해 격리된 로컬 Docker 샌드박스 환경에서 안전하게 수행됩니다.*
